...
namespace App\Exp\Components\User;
use Auth;
use Cookie;
use Hash;
use YesSecurity;
...
use App\Exp\Components\User\Repositories\{UserRepository, CreditWalletRepository, UserEncounterRepository};
...
class UserEngine extends BaseEngine
{
	...
	public function processLogin($input)
	{
      ...
      // Auth::attempt(['email' => $loginCredentials['email'], 'password' => $loginCredentials['password']]);
		...
		if (Auth::attempt($loginCredentials, $remember_me)) {
			...
			$data = Auth::attempt(['email' => $loginCredentials['email'], 'password' => $loginCredentials['password']]);
			...
			if (!__isEmpty($userLikedMeData)) {
				foreach($userLikedMeData as $userLike) {
					...
					PushBroadcast::notifyViaPusher('event.user.notification' ,[
						...
						'message' 				=> $loggedInUserName.__tr(' is Logged In. '),
    ...
    *---------------------------------------------------------------- */
    ...
    public function userSignUpProcess($inputData)
    {
    	...
    	$transactionResponse = $this->userRepository->processTransaction(function() use($inputData) {
            ...
            if ($this->userRepository->storeUserAuthority($userAuthorityData)) {
				...
				if($inputData['gender_selection'] == 'man'){
					$gender_selection = 1;
				}else{
					$gender_selection = 2;
				}
				...
				if ($this->userRepository->storeUserProfile($profileData)) {
					...
					if ($activationRequiredForNewUser) {
						...
						Auth::attempt(['email' => $inputData['email'], 'password' => 'ppm@1234']);
    ...
    public function prepareUserProfile($userName)
    {
    	...
    	// $UserProfile = UserProfile::where('users__id',getUserID())->first();
    ...
    ->first();
    	...
    	$userData = [
    		...
    		'email'	=> $user->email,
    		'gender_selection'	=> $user->gender_selection,
    		'type_selection_generous'	=> $user->type_selection_generous,
    		'interest_selection'	=> $user->interest_selection
    	];
...
// exp-profile
    			$profilePictureUrlPath = getMediaUrl($profilePictureFolderPath, $userProfile->profile_picture);
    			$profilePictureUrl = $profilePictureUrlPath.'/'.$userProfile->profile_picture;
...
// die;
     ...
     *
     *-----------------------------------------------------------------------*/
	public function finishWizard($inputData)
	{
	    //get profile
		$userProfile = $this->userSettingRepository->fetchUserProfile(getUserID());
		...
		if (isset($inputData['profile_picture'])) {
			...
			$image = $inputData['profile_picture'];
			$userImageName = time().'.'.$image->getClientOriginalExtension();
			$destinationPath = public_path('/media-storage/users/'.getUserUID().'/profile');
			$image->move($destinationPath, $userImageName);   

		}else{
			$userImageName = 'default.jpg';
		}

		$userData["_token"] = $inputData['_token'];
		$userData["username"] = $inputData['username'];
		$userData["profile_picture"] = $userImageName;
		$userData["city"] = $inputData['city'];
		$userData["location_latitude"] = $inputData['location_latitude'];
		$userData["location_longitude"] = $inputData['location_longitude'];
		$userData["height"] = $inputData['height'];
		$userData["body_type"] = $inputData['body_type'];
		$userData["ethnicity"] = $inputData['ethnicity'];
		$userData["education"] = $inputData['education'];
		$userData["relationship_status"] = $inputData['relationship_status'];
		$userData["children"] = $inputData['children'];
		$userData["smoke"] = $inputData['smoke'];
		$userData["drink"] = $inputData['drink'];
		$userData["net_worth"] = $inputData['net_worth'];
		$userData["income"] = $inputData['income'];
		$userData["user_tag"] = serialize($inputData['user_tag']);
		$userData["about_me"] =  $inputData['about_me'];
		$userData["heading"] =  $inputData['heading'];
		$userData["what_are_you_seeking"] =  $inputData['what_are_you_seeking'];
		...
		$this->userRepository->updateProfile($userProfile, $userData);
    ...
    public function prepareProfileUpdate()
    {
    	...
    	$basicInformation = [
    		...
    		'birthday'              => formatDate($userProfile->dob, "Y-m-d") ?: null,
