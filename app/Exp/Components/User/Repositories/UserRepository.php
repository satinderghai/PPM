...
namespace App\Exp\Components\User\Repositories;
use Str;
use Auth;
use Request;
use DB;
use App\Exp\Base\BaseRepository;
use App\Exp\Components\User\Blueprints\UserRepositoryBlueprint;
...
use App\Exp\Components\User\Models\LoginAttempt;
use YesSecurity;
...
class UserRepository extends BaseRepository implements UserRepositoryBlueprint
{
    ...
    * @return eloquent collection object
    *---------------------------------------------------------------- */
    public function fetchProfileVisitorData()
    {
		return ProfileVisitorModel::leftjoin('users', 'profile_visitors.by_users__id', '=', 'users._id')
								->leftjoin('user_authorities', 'users._id', '=', 'user_authorities.users__id')
								->leftjoin('user_profiles', 'users._id', '=', 'user_profiles.users__id')
    ...
    *-----------------------------------------------------------------------*/
    ...
    public function storeUser($storeData)
    {
        ...
        $keyValues = [
            ...
            'gender_selection' => $storeData['gender_selection'],
            'interest_selection' => $storeData['interest_selection'],
            'type_selection_generous' => $storeData['type_selection_generous'],
            ...
            // 'first_name',
            // 'last_name',
            'username' => Str::slug($storeData['email'])
    ...
    public function storeUserProfile($inputData)
    {
        $keyValues = [
            ...
            'dob' 			=> date("Y-m-d",strtotime($inputData['dob'])),
