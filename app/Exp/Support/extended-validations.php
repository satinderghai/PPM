...
use Carbon\Carbon;
    ...
    Validator::extend('unique_title', function($attribute, $value, $parameters)
    {
        ...
        $account = App\Exp\Services\ServiceControl\Models\AccountModel::where('title', $title)->first();
    ...
    Validator::extend('unique_email', function($attribute, $value, $parameters)
    {
        ...
        $user = App\Exp\Components\User\Models\User::where('email', $email)->first();
        $accountUser = null;
        ...
        if (!__isEmpty($user)) {
            ...
            $accountUser = App\Exp\Components\User\Models\UserAuthorityModel::where([
                'users__id' => $userId
            ])->first();
        }
    ...
    Validator::extend('ssh_public_key', function ($attribute, $value, $parameters, $validator) {
        return App\Exp\Support\Utils::validatePublicKey($value);
    });
    ...
    {   
        ...
        $twoFactorAuthKey = App\Exp\Components\User\Models\UserModel::where('email', $email)->pluck('2fa_enabled')->toArray();
    ...
    {    
        ...
        $title = Str::slug($value, '-');
		$page = App\Exp\Components\Pages\Models\PageModel::where('title', $inputData['title'])->where('_uid', '!=', $inputData['pageUid'])->first();
		// check is empty
        if (!__isEmpty($page)) {
            return false;
        }
